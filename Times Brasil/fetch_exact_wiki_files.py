...
headers = {'User-Agent': 'AntigravityBot/3.0 (https://example.org; bot@example.org) Python/3.14'}

targets = [
    ("Futebol Arabia Saudita", "02", "kings_cup.png", "King Cup (Copa do Rei)", "File:Saudi Arabian Football Federation Logo.svg"),
    ...
    ("Futebol Bolivia", "01", "primera_division_bolivia.png", "Primera División de Bolivia", "File:Federación Boliviana de Fútbol logo.svg"),
    ("Futebol Bolivia", "02", "copa_division_profesional.png", "Copa División Profesional", "File:Federación Boliviana de Fútbol logo.svg")
]

raw_tmp = "temp_exact_wiki.png"

for country_folder, num, fn, name, wiki_file in targets:
    print(f"[{country_folder}] {name}...", end=" ", flush=True)
    time.sleep(1.0)
    
    api_url = f"https://en.wikipedia.org/w/api.php?action=query&titles={urllib.parse.quote(wiki_file)}&prop=imageinfo&iiprop=url&iiurlwidth=500&format=json"
    img_url = None
    try:
