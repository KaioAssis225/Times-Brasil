...
search_queries = [
    ("Futebol Egito", "03", "egyptian_super_cup.png", "Egyptian Super Cup", "Egyptian Football Association logo"),
    ("Futebol Peru", "02", "copa_bicentenario.png", "Copa Bicentenario", "Federacion Peruana de Futbol logo"),
    ("Futebol Bolivia", "01", "primera_division_bolivia.png", "Primera División de Bolivia", "Federacion Boliviana de Futbol logo"),
    ("Futebol Bolivia", "02", "copa_division_profesional.png", "Copa División Profesional", "Federacion Boliviana de Futbol logo")
]

raw_tmp = "temp_fed_fix.png"

for country_folder, num, fn, name, query in search_queries:
    print(f"[{country_folder}] {name}...", end=" ", flush=True)
    time.sleep(1.0)
    
    # Query EN Wikipedia search
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            s_res = data.get("query", {}).get("search", [])
            for res in s_res:
                t_title = res.get("title", "")
                if t_title.startswith("File:") and not any(b in t_title.lower() for b in [".pdf", ".webm"]):
                    info_url = f"https://en.wikipedia.org/w/api.php?action=query&titles={urllib.parse.quote(t_title)}&prop=imageinfo&iiprop=url&iiurlwidth=500&format=json"
                    ...
                    with urllib.request.urlopen(info_req, timeout=10) as info_resp:
                        ...
                        for p_id, p_val in pages.items():
                            ...
                            if ii:
                                ...
                                print(f"Found {t_title}", end=" -> ", flush=True)
                                break
                    if img_url:
                        break
